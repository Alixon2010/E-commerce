...
from Shop.models import Card, Order, Product, Transaction
...
class TestStripeWebhookView(APITestCase):
    def setUp(self):
        self.client = APIClient()
        self.user = User.objects.create_user(
            username="wuser", email="w@example.com", password="pass123"
        )
        ...
        self.order = Order.objects.create(
            ...
            latitude=41.0,
            longitude=69.0,
            stripe_payment_intent="pi_web_1",
            paid=False,
        )
    ...
    def test_webhook_payment_succeeded(self, mock_construct):
        ...
        assert resp.json().get("status") == "muvaffaqiyatli"
    ...
    def test_webhook_payment_failed(self, mock_construct):
        ...
        assert resp.status_code == 200
        assert resp.json().get("status") == "xato"
