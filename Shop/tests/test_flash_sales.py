...
from Shop.models import FlashSales, Product
...
class FlashSalesViewsTests(APITestCase):
    ...
    def test_add_products_success(self):
        ...
        with patch(
            "Shop.views.FlashSales.objects.get", return_value=mock_flash
        ) as mock_get:
            request = self.factory.post(
                "", {"products": [str(self.product.id)]}, format="json"
            )
            view = FlashSaleAddProductsView.as_view()
            response = view(request, pk=1)

            assert response.status_code == 200
            assert response.data.get("status") == "mahsulotlar qo'shildi"
            mock_products_rel.add.assert_called()
            mock_get.assert_called_once_with(pk=1)

    def test_add_products_not_found(self):
        with patch(
            "Shop.views.FlashSales.objects.get", side_effect=FlashSales.DoesNotExist
        ):
            request = self.factory.post(
                "", {"products": [str(self.product.id)]}, format="json"
            )
            ...
            assert response.data.get("error") == "FlashSale topilmadi"
